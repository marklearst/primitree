...
describe("useVariableModes", () => {
  ...
  it("should return processed modes when useVariables has data", () => {
    ...
    expect(result.current.modesByCollectionId[collection1.id]).toEqual(collection1.modes);
    expect(result.current.modesByCollectionId[collection2.id]).toEqual(collection2.modes);
    expect(result.current.modesById[collection1.modes[0]!.modeId]).toEqual(collection1.modes[0]);
    expect(result.current.modesById[collection2.modes[0]!.modeId]).toEqual(collection2.modes[0]);
  });
