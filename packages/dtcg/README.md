...
```sh
npm install @primitree/dtcg@next
```
...
lowercasing and Unicode normalization. The emitted path, including the fixed
`tokens/` directory, can contain at most 64 nested directory levels, 16,639
UTF-8 bytes in total, and 255 UTF-8 bytes per segment. This leaves up to 63
nested directory levels in a token file name. File names cannot contain a lone
UTF-16 surrogate. The JSON sorter
