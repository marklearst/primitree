...
export async function runInspect(args: ParsedArgs): Promise<void> {
  ...
  const tokenPathInput = args.positionals[0]
  if (
    args.positionals.length !== 1 ||
    tokenPathInput === undefined ||
    tokenPathInput.length === 0
  ) {
    throw new Error('Inspect needs one token path.')
  }
  for (const flag of Object.keys(args.flags)) {
    if (!INSPECT_FLAGS.has(flag)) {
  ...
  const tokenPath = tokenPathInput.split('.')
  ...
  const describeToken = (tokenId: TokenId) => {
    const tokenPath = paths.get(tokenId)
    if (tokenPath === undefined) {
      throw new Error(`Token "${tokenId}" is outside the configured source.`)
    }
    return Object.freeze({ id: tokenId, path: tokenPath })
  }
