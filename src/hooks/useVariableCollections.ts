...
const useVariableCollections = (fileKey: string): HookState<FigmaCollection[]> => {
  ...
  useEffect(() => {
    const fetchCollections = async () => {
      ...
      try {
        ...
        setState({ data: data.meta.variableCollections, loading: false, error: null });
      } catch (error) {
        setState(s => ({ ...s, loading: false, error: error instanceof Error ? error : new Error('Failed to fetch') }));
      }
    };
    fetchCollections();
  }, [fileKey, token]);
